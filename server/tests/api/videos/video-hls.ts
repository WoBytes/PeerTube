...
async function checkHlsPlaylist (servers: ServerInfo[], videoUUID: string) {
  ...
  for (const server of servers) {
    ...
    expect(videoDetails.playlists).to.have.lengthOf(1)

    const hlsPlaylist = videoDetails.playlists.find(p => p.type === VideoStreamingPlaylistType.HLS)
...

    {
      ...
      for (const resolution of resolutions) {
        expect(masterPlaylist).to.contain(`http://localhost:9001/static/playlists/hls/${videoUUID}/${resolution}.m3u8`)
      }
    }
